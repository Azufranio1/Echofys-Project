  ...
  return (
    <>
      <style>{`
        ...
        .player-song-info { display:flex; align-items:center; gap:10px; width:30%; min-width:0; }
        .player-song-clickable { display:flex; align-items:center; gap:10px; flex:1; min-width:0; cursor:pointer; }
        .player-song-clickable:hover .player-song-title { color:#a78bfa; }
        .player-expand-hint { display:flex; align-items:center; color:#4B5563; transition:color 0.2s,transform 0.2s; flex-shrink:0; }
        ...
        .player-artwork {
          ...
          border:1px solid rgba(139,92,246,0.3); transition:box-shadow 0.4s; overflow:hidden;
        }
        .player-artwork.playing { box-shadow:0 0 0 1px rgba(139,92,246,0.4),0 0 20px rgba(139,92,246,0.35); }
        .player-artwork img { width:100%; height:100%; object-fit:cover; }
        .player-song-title { font-size:13px; font-weight:600; color:white; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; margin-bottom:3px; transition:color 0.2s; }
        .player-song-artist { font-size:11px; color:#6B7280; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
        .player-center { display:flex; flex-direction:column; align-items:center; gap:10px; flex:1; max-width:42%; }
        .player-controls { display:flex; align-items:center; gap:22px; }
        .player-btn-skip { background:none; border:none; cursor:pointer; color:#4B5563; display:flex; align-items:center; justify-content:center; transition:color 0.15s,transform 0.15s; padding:4px; border-radius:50%; }
        .player-btn-skip:hover { color:#E5E7EB; transform:scale(1.1); }
        .player-btn-play { width:40px; height:40px; border-radius:50%; border:none; cursor:pointer; display:flex; align-items:center; justify-content:center; background:linear-gradient(135deg,#9b72f5,#6d28d9); color:white; transition:transform 0.15s,box-shadow 0.3s; box-shadow:0 2px 10px rgba(109,40,217,0.4); }
        ...
        .player-time { font-size:10px; color:#6B7280; font-variant-numeric:tabular-nums; width:30px; flex-shrink:0; text-align:center; }
        .player-bar-wrap { flex:1; position:relative; height:4px; cursor:pointer; border-radius:4px; }
        .player-bar-wrap:hover .player-thumb { opacity:1; }
        .player-bar-track { position:absolute; inset:0; border-radius:4px; background:rgba(255,255,255,0.08); overflow:hidden; }
        .player-bar-fill { height:100%; border-radius:4px; background:linear-gradient(90deg,#7c3aed,#a78bfa); transition:width 0.1s linear; }
        .player-thumb { position:absolute; top:50%; transform:translate(-50%,-50%); width:12px; height:12px; border-radius:50%; background:white; box-shadow:0 0 8px rgba(139,92,246,0.9); opacity:0; transition:opacity 0.15s; pointer-events:none; }
        .player-range { position:absolute; inset:-8px 0; width:100%; height:calc(100% + 16px); opacity:0; cursor:pointer; }
        .player-right { display:flex; align-items:center; gap:8px; width:28%; justify-content:flex-end; }
      ...
      <div className="player-root">
        ...
        <div className="player-song-info">
          ...
          <HeartButton songId={currentSong._id} size={17} />
        </div>

        {/* CENTER */}
        <div className="player-center">
          <div className="player-controls">
            <button className="player-btn-skip" onClick={handleSkipBack}><SkipBack size={19} fill="currentColor"/></button>
            ...
            <button className="player-btn-skip" onClick={handleSkipForward}><SkipForward size={19} fill="currentColor"/></button>
          </div>
          <div className="player-progress-row">
            <span className="player-time">{formatTime(currentTime)}</span>
            <div className="player-bar-wrap">
              <div className="player-bar-track"><div className="player-bar-fill" style={{width:`${prog}%`}}/></div>
              <div className="player-thumb" style={{left:`${prog}%`}}/>
              <input type="range" className="player-range" min={0} max={duration||0} value={currentTime}
        ...
        <div className="player-right">
          ...
          <div className="player-bar-wrap" style={{width:'88px'}}>
            <div className="player-bar-track"><div className="player-bar-fill" style={{width:`${volDisplay}%`}}/></div>
            <div className="player-thumb" style={{left:`${volDisplay}%`}}/>
            <input type="range" className="player-range" min={0} max={100} value={volDisplay}
