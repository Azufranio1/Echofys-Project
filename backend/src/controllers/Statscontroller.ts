...
export const getUserStats = async (req: Request, res: Response) => {
  try {
    ...
    // Géneros última semana
    const weekAgo       = Date.now() - 7 * 24 * 3600 * 1000;
    const recentSongIds = plays
      .filter((p: any) => new Date(p.playedAt).getTime() > weekAgo)
      .map((p: any) => p.songId.toString());
    ...
    const heatmapHours = Array.from({ length: 24 }, (_, h) => ({ hour: h, count: hourCount[h] || 0 }));
    ...
    plays.forEach((p: any) => {
      const d = new Date(p.playedAt).getDay();
      dayCount[d] = (dayCount[d] || 0) + 1;
    });
    const heatmapDays = Array.from({ length: 7 }, (_, d) => ({ day: DAY_NAMES[d], count: dayCount[d] || 0 }));
    ...
    const explorerPct    = allGenres.length > 0 ? Math.round((listenedGenres.size / allGenres.length) * 100) : 0;
    ...
    const result = {
      ...
      minutesTotal:         Math.round(uniqueIds.length * 3.5),
      topSongs,
      topArtists,
      genreDistribution,
      personality,
...
// ── Helpers ──
function getPersonality(genre: string, pct: number) {
...
function getTimePersona(hour: number) {
...
function getExplorerLevel(pct: number) {
